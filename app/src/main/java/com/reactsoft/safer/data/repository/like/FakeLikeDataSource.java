...
public class FakeLikeDataSource implements LikeDataSource {
  ...
  @Override
  public ObservableTask<List<Like>> getLikes(final String photoId) {
    return new ObservableTask<List<Like>>() {
      @Override
      public void run(Subscriber<List<Like>> result) {
  ...
  @Override
  public ObservableTask<Boolean> toggleLike(@NonNull final Like like) {
